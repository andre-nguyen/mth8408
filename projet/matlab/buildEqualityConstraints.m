...
n_coeffs = n+1;       % number of coefficients in a polynomial
n_states = 3;         % x y z
I = eye(n_coeffs);    % Identity matrix for compulations

% Build A_0 and b_0, the constraints relating to the waypoints and its
% derivatives,
A_0 = zeros(2*n_states*m, n_states*n_coeffs*m);
b_0 = zeros(2*n_states*m, 1);

for i = 1:m % For each position waypoint
    % "A is constructed by evaluating the component of the derivative in
    % equation 9"
    waypoint = w(1, i, :);
    waypoint = reshape(waypoint, n_states, 1);
    values = zeros(1, n_coeffs);
    if i == 1 % Initial conditions
        ...
        for j = 1:n_states
            a = zeros(1, n_states * n_coeffs * m);
            lower_idx =  (i-1) * n_coeffs * n_states + (j-1) * n_coeffs +1;
            upper_idx = ((i-1) * n_coeffs * n_states + (j-1) * n_coeffs   ) + n_coeffs;
            a(lower_idx : upper_idx) = values;
            A_0(j, :) = a;
        end
        b_0(1:n_states) = waypoint;
        ...
        for j = 1:n_states
            a = zeros(1, n_states * n_coeffs * m);
            lower_idx =  (i-2) * n_coeffs * n_states + (j-1) * n_coeffs +1;
            upper_idx = ((i-2) * n_coeffs * n_states + (j-1) * n_coeffs   ) +n_coeffs;
            a(lower_idx : upper_idx) = values;
            A_0(j + 2 * n_states * (i-1), :) = a;
        end
        b_0(2 * n_states * (i-1) + 1 : ...
            2 * n_states * (i-1) + n_states) = waypoint;
        ...
        for j = 1:n_states
            ...
            upper_idx = ((i-1) * n_coeffs * n_states + (j-1) * n_coeffs   ) +n_coeffs;
            a(lower_idx : upper_idx) = values;
            A_0(j + 2 * n_states * (i-1) + n_states, :) = a;
        end
        b_0(2 * n_states * (i-1) + n_states + 1: ...
            2 * n_states * (i-1) + 2*n_states) = waypoint;
    end
end

% Derivative constraints (without yaw)
A_T = zeros(2 * m * (n_states-1) * k_r, n_states * n_coeffs * m);
b_T = ones(2 * m * (n_states-1) * k_r, 1) * eps;
...
for i = 1:m     % For each waypoint
    for j = 1:k_r   % For each derivative of the position states
        ...
        if i == 1
            ...
            for k = 1:n_states-1  % For states x y z (no yaw)
                a = zeros(1, n_states * n_coeffs * m);
                % Note
                % w rows: derivatives
                %   cols: waypoints
                ...
                if constraint == Inf
                    ...
                    % starting
                    lower_idx =  (i-1) * n_coeffs * n_states + (k-1) * n_coeffs + 1;
                    upper_idx = ((i-1) * n_coeffs * n_states + (k-1) * n_coeffs    ) + n_coeffs;
                    a(lower_idx : upper_idx) = values;
                    lower_idx =  (m-1) * n_coeffs * n_states + (k-1) * n_coeffs + 1;
                    upper_idx = ((m-1) * n_coeffs * n_states + (k-1) * n_coeffs    ) + n_coeffs;
                    a(lower_idx : upper_idx) = -values;
                    A_T(k + (j-1) * (n_states-1), :) = a;
                    b_T(k + (j-1) * (n_states-1)) = 0;
                else
                    ...
                    a(lower_idx : upper_idx) = values;
                    A_T(k + (j-1) * (n_states-1), :) = a;
                    b_T(k + (j-1) * (n_states-1)) = constraint;
                end
            end
            ...
            for k = 1:n_states-1
                ...
                if constraint == Inf
                    ...
                    lower_idx =  (i-2) * n_coeffs * n_states + (k-1) * n_coeffs +1;
                    upper_idx = ((i-2) * n_coeffs * n_states + (k-1) * n_coeffs   ) + n_coeffs;
                    a(lower_idx : upper_idx) = values;
                    lower_idx =  (i-1) * n_coeffs * n_states + (k-1) * n_coeffs +1;
                    upper_idx = ((i-1) * n_coeffs * n_states + (k-1) * n_coeffs   ) + n_coeffs;
                    a(lower_idx : upper_idx) = -values;
                    A_T(k + (j-1) * (n_states-1) + 2 * (i-1) * (n_states-1) * k_r, :) = a;
                    b_T(k + (j-1) * (n_states-1) + 2 * (i-1) * (n_states-1) * k_r) = 0;
                else
                    % We have a constraint
                    lower_idx =  (i-2) * n_coeffs * n_states + (k-1) * n_coeffs +1;
                    upper_idx = ((i-2) * n_coeffs * n_states + (k-1) * n_coeffs   ) + n_coeffs;
                    a(lower_idx : upper_idx) = values;
                    A_T(k + (j-1) * (n_states-1) + 2 * (i-1) * (n_states-1) * k_r, :) = c;
                    b_T(k + (j-1) * (n_states-1) + 2 * (i-1) * (n_states-1) * k_r) = constraint;
                end
            end
        end
    end
