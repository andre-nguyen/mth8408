...
#endif // ifdef USE_GUROBI
#ifdef USE_QPOASES
# include <qpOASES/QProblem.hpp>
#endif // ifdef USE_QPOASES
...
MatrixXd TrajectoryGenerator::getConstraintMatrix(int dim) const {
  MatrixXd A_temp(
    A_fixed_[dim].rows() + A_continuity_[dim].rows(), A_fixed_[dim].cols());
...
double TrajectoryGenerator::getObjectiveFuncVal() const {
    ...
    for(int i = 0; i < STATE_COUNT; ++i) {
        result += solution_[i].transpose() * H_[i] * solution_[i];
    }
    return result;
}
...
bool TrajectoryGenerator::solveProblem(Solver solver) {
  ...
  // YOLO alpha is 1
  for (int wp = 0; wp < getNumWaypoints() - 1; ++wp) {
    TrajectorySegment ts(keyframes_[wp].getTime(),
                         keyframes_[wp + 1].getTime(), 1,
...
bool TrajectoryGenerator::solveProblemOoqp(int dim) {
   ...
   *  Find x: min 1/2 x' Q x + c' x such that A x = b, d <= Cx <= f, and l <= x
   *<= u
   */
...
bool TrajectoryGenerator::solveProblemGurobi(int dim) {
  ...
  MatrixXd    Q = H_[dim] +
                  (1e-2 * MatrixXd::Identity(H_[dim].rows(), H_[dim].cols()));
  ...
  /*
     ...
     for(i = 0; i < rows; ++i) {
      if(constrs[i].get(GRB_IntAttr_IISConstr) > 0)
          std::cout << "Guilty: " << constrs[i].get(GRB_StringAttr_ConstrName)
             << std::endl;
     }
     std::cout << "WTF\n";
...
bool TrajectoryGenerator::solveProblemQuadprog(int dim) {
  Eigen::QuadProgDense qp(H_[dim].rows(),
                          b_fixed_[dim].rows() + b_continuity_[dim].rows(),
                          0);
...
bool TrajectoryGenerator::solveProblemIPOPT(int dim) {
  QuadraticProgramming *qp =
    new QuadraticProgramming(H_[dim].cols(), b_fixed_[dim].rows() +
                             b_continuity_[dim].rows());
...
void TrajectoryGenerator::buildCostMatrix(int dim) {
  ...
  for (int wp = 0; wp < wps; ++wp) {
    ...
    /**
     * TODO check if I should be using a sparse matrix and if there is a
     *difference
...
void TrajectoryGenerator::buildConstraintMatrix(int dim) {
  ...
  for (int wp = 0; wp < wps; ++wp) {
    // We don't go to the snap because that's what we want to minimize I
    // think...
    for (int der = Derivative::DER_POSITION; der < Derivative::DER_SNAP; ++der) {
      // Don't add a constraint if it's unconstrained, duh!
      if (!keyframes_[wp].isConstrained(der, dim)) continue;
  ...
  assert(A_eq.size() == b_eq.size()); // Would be pretty awks if this wasn't
                                      // true
  ...
  for (int wp = 1; wp < wps - 1; ++wp) {
    for (int der = Derivative::DER_POSITION; der < Derivative::DER_COUNT;
         ++der) {
